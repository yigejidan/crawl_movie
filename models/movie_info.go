...
func init(){
	orm.Debug = true //是否开启调试模式，调试模式下回打印出sql语句
	orm.RegisterDataBase("default","mysql","root:105153@tcp(127.0.0.1:3306)/test?charset=utf8",30)
...
func GetMovieCountry(movieHtml string) string {
	...
	reg := regexp.MustCompile(`<span class="pl">制片国家/地区:</span>
								(.*?)
								<br>`)
	result := reg.FindAllStringSubmatch(movieHtml,-1)
	if len(result) == 0 {
...
func GetMovieLanguage(movieHtml string) string {
	...
	reg := regexp.MustCompile(`<span class="pl">语言:</span>
								(.*?)
								<br>`)
	result := reg.FindAllStringSubmatch(movieHtml,-1)
	if len(result) == 0 {
